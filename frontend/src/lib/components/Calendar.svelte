<script lang="ts">
    ...
    let overflowDialog = $state<HTMLDialogElement | null>(null);
    ...
    $effect(() => {
        ...
        if (overflowDayKey) overflowDialog.showModal();
        else if (overflowDialog.open) overflowDialog.close();
    });

    function overflowDateLabel(key: string): string {
...
<div class="space-y-6">
    ...
    <!-- svelte-ignore a11y_no_noninteractive_element_interactions -->
    <dialog
        bind:this={overflowDialog}
        onclose={() => (overflowDayKey = null)}
        onclick={(e) => e.target === overflowDialog && (overflowDayKey = null)}
        ...
        {#if overflowDayKey}
            <div class="bg-[var(--bg)] border border-[var(--border)] rounded-xl shadow-xl p-5 mx-4 space-y-4">
                <div class="flex items-center justify-between">
                    <h2 class="type-body font-semibold text-[var(--text-1)]">{overflowDateLabel(overflowDayKey)}</h2>
                    <button
                        onclick={() => (overflowDayKey = null)}
