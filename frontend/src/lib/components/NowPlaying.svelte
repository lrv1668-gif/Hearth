<script lang="ts">
  ...
  const progressPct = $derived(
    track
      ? Math.min(100, ((track.progress_ms + (track.is_playing ? tickMs : 0)) / track.duration_ms) * 100)
      : 0
  );
</script>
...
{:else if track !== null}
  <div class="flex items-center gap-3 p-3 rounded-xl bg-[var(--surface)] border border-[var(--border)] min-w-0 group relative">
    ...
    <div class="flex flex-col min-w-0 gap-1 flex-1">
      <div class="min-w-0">
        <p class="text-sm font-medium text-[var(--text-1)] truncate leading-tight">{track.title}</p>
