...
<footer class="hidden shrink-0 border-t-2 border-[var(--border)] px-6 py-2 md:px-8 lg:block {extraClass}">
    <div class="flex items-center justify-center gap-2">
        {#if spotifyStore.nowPlaying}
            <span
                class="type-label flex min-w-0 items-center gap-1.5 truncate rounded-lg bg-[var(--surface)] p-2 text-[var(--text-2)]"
            >
                <Music class="icon-sm flex-shrink-0" />
                <span class="font-bold text-[var(--text-1)]">{spotifyStore.nowPlaying.title}</span>
                <span>{spotifyStore.nowPlaying.artist}</span>
            </span>
        {/if}

        {#if weatherStore.current?.fetched_at}
            <div class="type-label flex flex-shrink-0 items-center gap-2 rounded-lg bg-[var(--surface)] p-2">
                <Monitor class="icon-sm text-[var(--text-2)]]" />
