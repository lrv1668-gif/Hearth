...
>
    {#if dayKey}
        <div class="mx-4 space-y-4 rounded-2xl border border-(--border) bg-(--bg) p-5 shadow-xl">
            <div class="flex items-center justify-between">
                <h2 class="type-body font-semibold text-(--text-1)">{dateLabel(dayKey)}</h2>
                <button
                    onclick={close}
                ...
                {#each items as item (item.kind === 'task' ? `t-${item.data.id}` : `e-${item.data.id}`)}
                    {#if item.kind === 'task'}
                        ...
                        <li class="flex min-w-0 items-center gap-2">
                            ...
                            >
                                {#if task.due_time}
                                    <span class="type-label mr-1 text-(--text-3)">{formatTime(task.due_time)}</span>
                                {/if}
                                {task.title}
                            </button>
                        </li>
                        ...
                        {#if calItem.kind === 'task'}
                            <li class="flex min-w-0 items-center gap-2">
                                <button
                                    ...
                                    class="h-2 w-2 flex-shrink-0 rounded-full transition-colors
                                           {calItem.is_completed ? 'bg-(--done-bg)' : 'bg-(--accent) hover:opacity-80'}"
                                    aria-label="Toggle {calItem.title}"
                                ></button>
                                <span
                                    class="type-body min-w-0 flex-1 truncate
                                           {calItem.is_completed ? 'text-(--done) line-through' : 'text-(--text-1)'}"
                                >
                                    {calItem.title}
                                </span>
                                <span class="type-label flex-shrink-0 text-(--text-3)"
                                    >{providerLabel(calItem.provider)}</span
                                >
                            </li>
                        {:else}
                            <li class="flex min-w-0 items-center gap-2">
                                <CalendarIcon class="h-2 w-2 flex-shrink-0 text-(--accent)" aria-hidden="true" />
                                <button
                                    onclick={() => {
                                        close();
                                ...
                                >
                                    <span class="type-body block truncate text-(--text-1)">
                                        {#if !calItem.is_all_day && calItem.start}
                                            <span class="type-label mr-1 text-(--text-3)">
                                                {formatTime(calItem.start.slice(11, 16))}
                                            </span>
                                        {/if}
                                        {calItem.title}
                                    </span>
                                    <span class="type-label text-(--text-3)">{providerLabel(calItem.provider)}</span>
                                </button>
                            </li>
                        {/if}
                    {/if}
